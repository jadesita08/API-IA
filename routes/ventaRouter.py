...
status # APIRouter: agrupa endpoints de ventas 
...
try:
    if not manager.actualizar_venta(venta_id, venta): 
        ...
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, 
                            detail="Venta no encontrada o datos no actualizados") 
        return {"mensaje": f"Venta con ID {venta_id} 
        actualizado exitosamente"}
except Exception as e: 
    # Errores de entrada/validación
    raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, 
                        detail=f"Error al actualizar venta: {e}") 
...
try:
    ...
    # Error del servidor
    raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
detail=f"Error al eliminar venta: {e}")
