    ...
    try:
        ...
        if producto_id is None:
            ...
            HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, 
                          detail="Fallo de conexión o error al crear producto.")
...
try:
    if not manager.actualizar_producto(producto_id, producto):
        ...
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, 
                        detail="Producto no encontrado o datos no actualizados")
        return {"mensaje": f"Producto con ID {producto_id} actualizado
        exitosamente"}
        except Exception as e: 
            # Errores de entrada/validación
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, 
